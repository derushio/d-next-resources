...
setup:
	...
	pnpm add jose bcryptjs flowbite-react parse-duration zod date-fns lodash p-queue uuid @paralleldrive/cuid2 typescript-eslint zod-form-data react-icons next-auth react-responsive usehooks-ts
	pnpm add -D @dotenvx/dotenvx @trivago/prettier-plugin-sort-imports @types/bcryptjs @types/lodash @types/uuid eslint eslint-config-prettier npm-run-all prettier tsx @onozaty/prisma-db-comments-generator
